...
<?php 
	if(isset($_POST['submit_stream']))
	{
		...
		{	
			...
			$sql = "INSERT INTO semester_fees(stream, sem_1_fees,sem_2_fees,sem_3_fees,sem_4_fees,sem_5_fees,sem_6_fees,";
			$sql=$sql."sem_7_fees,sem_8_fees,no_of_sems,inst_id) VALUES ('". $amt[0]."',". $amt[1] .",".$amt[2].",".$amt[3]."," ;
			$sql=$sql. $amt[4].",". $amt[5] .",".$amt[6].",".$amt[7].",".$amt[8].",". $count_sem.",". $iid.")"; 
			//echo $amt[0]."-".$amt[1];
			if ($conn->query($sql) === TRUE) {
				echo "New record created successfully in semester_fees";
			}
			else {
